...
3. リポジトリ名・可視性を設定して **「Create repository from template」**
   - 無料プランの場合、GitHub Pages を使うにはリポジトリを **Public** にする必要があります
...
> **注意:** テンプレートからリポジトリを作成すると GitHub Actions が即時実行されますが、この設定が済んでいないためデプロイステップは失敗します。GitHub Pages を有効化したあと、Actions タブから失敗したワークフローを **「Re-run all jobs」** で再実行してください。
